    ...
    $("#sign-up-form").validate({
        ...
        rules: {
            ...
            maileCode: {
                ...
                minlength: 3,
                remote: {
                    url: "checkMailCode",     //后台处理程序
                    type: "post",               //数据发送方式
                    dataType: "json",           //接受数据格式
                    data: {                     //要传递的数据
                    	maileCode: function() {
                            return $("#maileCode").val();
                        }
                    }
                }
            },
        ...
        messages: {
        	mobile: {
                ...
                remote: "电话号码已被注册，请确认输入"
            },

            maileCode: {
                required: "验证码不能为空",
                minlength: "用户名至少3个字符",
                remote: "用户名已经被注册，请确认输入"
            },
            password: {
                required: "密码不能为空",
                noblank:true,
...
$("#sign-on-ra").on(ace.click_event, function() {
    bootbox.dialog({
        ...
        "            </p>" +
        "            <h5>一、网站使用说明：</h5>" +
        "               <p> 1、 本系统是由云物益邦根据中华人民共和国法律成立的网站。<br>" +
        ...
        "            </p>" +
        "                <h5>二、注册用户权利：</h5>" +
        "                <p>1. 云物益邦遵守国家法律法规，保护每一位注册用户的合法权利公平对待每一位注册用户，所有用户一律平等。<br>" +
        "                2. 云物益邦遵守对您的所有承诺，如有本系统赠送的优惠券和积分等，在活动有效期内，云物益邦将严格按照活动规则执行。<br>" +
        "                3. 云物益邦尊重用户的肖像权及对发布内容的所有权，未经您的同意，云物益邦将不用作任何商业用途。<br>" +
        "                4. 云物益邦尊重用户的隐私权，您的个人信息只有在下述情况中部分或全部被披露：<br>" +
        "                1) 经您同意，向第三人披露。<br>" +
        ...
        "                5. 您须对在本系统的活动及发布内容持客观和公正的态度并不得发布含有有损云物益邦形象或违反相关活动精神的内容。<br>" +
        ...
        "                8、 对侵害云物益邦用户权益的信息，请向网站管理员举报。<br>" +
        "                对于本系统禁止发布的内容，一经发现，本系统有权将其进行编辑或者直接删除，如有触犯法律的行为，将移送司法机关处理。" +
        "            </p>" +
        "                <h5>四、 知识产权：</h5>" +
        "                <p>1、 本系统上的所有内容，如商标、文字、图片、标识、广告都是云物益邦或者其内容提供者的排他性财产，您不可以未经授权擅自使用，同时，这些内容不可以被用于任何可能引起消费者对云物益邦的服务引起混淆或者贬低的商品或者服务上。<br>" +
        "                2、 未经云物益邦书面同意，您不可以用任何方式对本系统内容进行修改，也不可复制、下载本系统网页内容、数据、信息进行转售或商业利用。" +
        "            </p>" +
        "               <h5> 五、 免责条款：</h5>" +
        "                <p>1、 虽然本系统已将所有信息和资料进行过认真编辑与审查，但是仍然可能存在错误或不准确的信息。本系统不保证以下各项：信息和资料中包含的功能将不受干扰或没有错误；信息和资料将符合用户的特定要求；缺陷将被纠正；本系统或提供本系统的服务器没有病毒或其它有害成分；通过本系统传送的讯息将能维持保密。就本系统中信息和资料的正确性、准确性、可靠性或其它方面而言，云物益邦概不对本系统中的信息和资料的使用或使用后果做出任何保证。<br>" +
        "                2、 对于因病毒、系统缺陷、人对计算机系统的作为或不作为、电话线、硬件、软件、程序出错或其它由于使用本系统而引起的计算机传输或网络连接错误、失败或延迟造成的损害，本系统也不承担责任，即便这些损害是由于用户登陆或使用本系统。云物益邦不保证本系统的使用是持续的、不中断的或安全的。<br>" +
        "                3、 由于您的的保管疏忽导致帐号、密码遭他人非法使用。云物益邦不承担任何法律责任。<br>" +
        "                4、 云物益邦对本系统任何可能含有的第三方网站的内容、隐私权政策或操作不享有控制权，亦不承担任何责任。此外，云物益邦亦无法审查或编辑任何第三方网站的相关内容。云物益邦概不负责因您使用本系统上连接的第三方网站所产生的任何全部责任。因此，我们希望您留意自己何时离开本系统并阅读您所访问的任一其他网站的使用条款与隐私权政策。" +
        "                </p>" +
        "                   <h5> 六、 其他：</h5>" +

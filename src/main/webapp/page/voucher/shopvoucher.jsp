                            ...
                            <div class="col-xs-12">
                                <table id="simple-table" class="table  table-bordered table-hover perm-list">
                                    ...
                                    <tr>
                                        ...
                                        <th><i class="fa fa-key bigger-110 hidden-480"></i>代金券名字</th>
                                    ...
                                    <tbody id="222">
                                    <shiro:hasPermission name="shopvoucher:list">
                                    <c:forEach items="${list.list}" var="list">
                                        ...
                                        <tr>
                                            ...
                                            <td class="vid">${list.vid}</td> 
                                            <td class="vtitle">${list.vtitle}</td> 
                                            <td class="vmoney">${list.vmoney}</td> 
                                            <td class="vprice">${list.vprice}</td>	
                                            <td class="vsdate">${findname}</td> 
                                            <td class="vodate">${list.vnum}</td> 
                                            <td class="vodate">${list.vstore}</td> 
                                            <td class="vgoods" id="aaa"> <input type="hidden" id="date" value="${list.vodate }"/>${list.vsdate}-${list.vodate }</td>                                             
                                                                                       	                                       
                                            
                                            <td>
                                                <div class="hidden-sm hidden-xs btn-group">

																<%-- <shiro:hasPermission name="shopvoucher:update"> --%>
																...
																<%-- </shiro:hasPermission> --%>
																<button type="button" class="btn btn-xs btn-info" onclick="zuofei(${list.vid})" <c:if test="${list.vstore ne list.vnum }">disabled="disabled"</c:if>>作废</button>
																<!-- </button><button type="button" class="list-group-item" display:inline>删除</button><button type="button" class="list-group-item" style="display:inline">作废</button> -->
                                    ...
                                    <shiro:lacksPermission name="shopvoucher:list">
                                    ...
                                    <ul class="pagination">
                                        <c:choose>
                                            <c:when test="${list.hasPreviousPage}">
                                                <li><a href="${path}/shopvoucher/list?pageNum=1">首页</a></li>
                                                <li>
                                                    <a href="${path}/shopvoucher/list?pageNum=${list.pageNum-1}"
                                                       aria-label="Previous">
                                        ...
                                        <c:forEach items="${list.navigatepageNums }" var="page">
                                            <c:if test="${page==list.pageNum}">
                                                <li class="active"><a
                                                        href="${path}/shopvoucher/list?pageNum=${page}">${page}</a></li>
                                            </c:if>
                                            <c:if test="${page!=list.pageNum}">
                                                <li><a href="${path }/shopvoucher/list?pageNum=${page}">${page}</a></li>
                                            </c:if>
                                        </c:forEach>
                                        ...
                                        <c:choose>
                                            <c:when test="${list.hasNextPage }">
                                                ...
                                                <li><a href="${path }/shopvoucher/list?pageNum=${list.pages}">末页</a></li>
                                            </c:when>
                                            <c:otherwise>
                                        ...
                                        <div class="modal-body">
                                            <div class="row">
                                                <div class="col-xs-12 col-sm-6">
                                                    <input type="hidden" name="vid" class="form-control">
                                                    <div class="form-group">                                                                                                          
                                                        ...
                                                        <div>
                                                            <input type="text" id="vrange" name="vrange" placeholder=""/>
                                                        </div>
                                                        
                                                        <label for="codeInput">总量<span style="color:#F00">*</span></label>
                                                        <div>
                                                            <input type="text" id="vnum" name="vnum" placeholder=""/>
                                                        </div>
                                                        
                                                        <label for="codeInput">库存<span style="color:#F00">*</span></label>
                                                        <div>
                                                            <input type="text" id="vstore" name="vstore" placeholder=""/>
                                                        </div>
                                                        
                                                        <label for="codeInput">生效日期<span style="color:#F00">*</span></label>
                                                        <div>
                                                            <input class="Wdate" onFocus="WdatePicker({lang:'zh-cn',dateFmt:'yyyy-MM-dd HH:mm:ss',Date:new Date(),})" name="vsdate"/>
                                                        </div>
                                                        
                                                        <label for="codeInput">失效日期<span style="color:#F00">*</span></label>
                                                        <div>
                                                            <input class="Wdate" onFocus="WdatePicker({lang:'zh-cn',dateFmt:'yyyy-MM-dd HH:mm:ss',Date:new Date(),})" name="vodate"/>
                                                        </div>
...
<!-- 与此页相关的内联脚本 -->
<script src="${path}/assets/custom-js/voucher/shopvoucher.js"></script>

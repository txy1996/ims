                        ...
                        <div class="row">
                            <div class="col-xs-12">
                                <table id="simple-table" class="table  table-bordered table-hover perm-list">
                                    ...
                                    <tbody id="222">
                                    <shiro:hasPermission name="uservoucher:list">
                                    <c:forEach items="${list.list}" var="list">
                                        
                                       
                                        
                                        <tr>
                                        	<input type="hidden" id="uvinfo1" value="${list.uvinfo}"/>
                                        	<input type="hidden" id="uvlInvalid1" value="${list.uvlInvalid}"/>
                                            <td class="vid">${list.uvlid}</td> 
                                            <td class="vmoney"><input type="hidden" id="hi" value="${list.vou.vid }"/>${list.vou.vtitle}</td> 
                                            <td class="vcost">${list.vou.vmoney}</td> 
                                            <td class="vprice">${list.vou.vprice}</td>	
                                            <td class="vsdate">${list.a}
                                            <c:set var="theString" value="${list.vou.vrange}"/>                                        
                                            <c:choose> 
                                            ...
                                            <td class="vodate"><input type="hidden" id="state1" value="${list.uvlstate}"/>${list.uvlstate}</td> 
                                            <td class="vgoods" id="aaa"> <input type="hidden" id="date" value="${list.vou.vodate }"/>${list.vou.vsdate}-${list.vou.vodate }</td> 
                                            <td class="vintegration">${list.sb.bname }[${list.s.sname }]</td> 
                                            <td class="vimg">${list.uvltype}</td> 
                                            ...
                                            <td>
                                                <div class="hidden-sm hidden-xs btn-group">
                    												...
                    												data-whatever="${list.uvlid}" <c:if test="${not empty list.uvlInvalid }">disabled="disabled"</c:if> <c:if test="${not empty list.s.sname }">disabled="disabled"</c:if>>作废
                    												
                    												</button>
																
																	<button type="button" class="btn btn-xs btn-info" data-toggle="modal" data-target="#exampleModal1"
                    												data-whatever="${list.uvlid}" <c:if test="${not empty list.uvlInvalid }">disabled="disabled"</c:if> <c:if test="${not empty list.s.sname }">disabled="disabled"</c:if>>延期
            														</button>
															</div>
                                            </td>
                                        </tr>
                                    ...
                                    <shiro:lacksPermission name="uservoucher:list">
                                    ...
                                    <ul class="pagination">
                                        <c:choose>
                                            <c:when test="${list.hasPreviousPage}">
                                                <li><a href="${path}/uservoucher/list?pageNum=1&urid=${urid1 }">首页</a></li>
                                                <li>
                                                    <a href="${path}/uservoucher/list?pageNum=${list.pageNum-1}&urid=${urid1 }"
                                                       aria-label="Previous">
                                        ...
                                        <c:forEach items="${list.navigatepageNums }" var="page">
                                            <c:if test="${page==list.pageNum}">
                                                <li class="active"><a
                                                        href="${path}/uservoucher/list?pageNum=${page}&urid=${urid1 }">${page}</a></li>
                                            </c:if>
                                            <c:if test="${page!=list.pageNum}">
                                                <li><a href="${path }/uservoucher/list?pageNum=${page}&urid=${urid1 }">${page}</a></li>
                                            </c:if>
                                        </c:forEach>
                                        ...
                                        <c:choose>
                                            <c:when test="${list.hasNextPage }">
                                                ...
                                                <li><a href="${path }/uservoucher/list?pageNum=${list.pages}&urid=${urid1 }">末页</a></li>
                                            </c:when>
                                            <c:otherwise>
...
function save1(){
	...
	console.log("333")
	$.ajax({
        url:"../uservoucher/update.html",
        type:"POST",
        data:{
        	uvlid:recipient,
        	uvinfo:$("#message-text").val()
        },
        success:function(){
...
function save(){
	...
	$.ajax({
        url:"../uservoucher/update1.html",
        type:"POST",
        data:{
        	uvlid:recipient,
        	uvinfo:$("#message-text1").val(),
        	time:$("#se").val(),
        	vid:$("#hi").val(),
        	state:$("#state1").val()
        },

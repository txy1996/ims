...
<body class="no-skin" id="Refreshbody">
		...
		<div class="main-content">
			...
			<div class="main-content-inner">
				<div class="breadcrumbs ace-save-state breadcrumbs-fixed"
					id="breadcrumbs">
					<ul class="breadcrumb">
						<li><i class="ace-icon fa fa-home home-icon"></i> <a href="#">商户积分列表</a>
						</li>
						<li class="active">商户积分列表</li>
					</ul>
					<!-- /.breadcrumb -->
				...
				<div class="page-content">
						...
						<h1>
							商户积分列表 <small> <i
								...
								共${list.total}个&emsp; <shiro:hasPermission
									name="slintegration:add">
									<button id="addslintegration" type="button" style="display: none;"
					...
					<div class="integral-user-filter ">
						...
						<select id="shopintegralitem" name="lclcid" style="height: 35px;">
							<option  >请选择...</option>
									...
									<table id="simple-table"
										...
										<thead>
											<tr>
												...
												<th><i class="fa  bigger-110 hidden-480"></i>获得原因</th>
												<th><i class="fa  bigger-110 hidden-480"></i>获得分值</th>
										...
										<tbody class="user-tbody">
											<shiro:hasPermission name="slintegration:list">
												<c:forEach items="${list.list}" var="list">
													<tr>
														<td style="display: none;" class="slid">${list.slid}</td>
														<td class="sname">${list.shop.sname}</td>
														<td class="sgetinfo">${list.sgetinfo}</td>
														<td class="sgetnum">${list.sgetnum}</td>
														<td class="sgetdate">${list.sgetdate}</td>
														<td class="ssum">${list.ssum}</td>
													</tr>
												</c:forEach>
											</shiro:hasPermission>
											<%--     <shiro:lacksPermission name="slintegration:show">
										...
										<ul class="pagination">
											<c:choose>
												<c:when test="${list.hasPreviousPage}">
													<li><a href="${path}/slintegration/list?pageNum=1&sid=${request.sid}">首页</a></li>
													<li><a
														href="${path}/slintegration/list?pageNum=${list.pageNum-1}&sid=${request.sid}"
											...
											<c:forEach items="${list.navigatepageNums }" var="page">
												<c:if test="${page==list.pageNum}">
													<li class="active"><a
														href="${path}/slintegration/list?pageNum=${page}&sid=${request.sid}">${page}</a></li>
												</c:if>
												<c:if test="${page!=list.pageNum}">
													<li><a
														href="${path }/slintegration/list?pageNum=${page}&sid=${request.sid}">${page}</a></li>
											...
											<c:choose>
												<c:when test="${list.hasNextPage }">
													...
													<li><a
														href="${path }/slintegration/list?pageNum=${list.pages}&sid=${request.sid}">末页</a></li>
												</c:when>
												<c:otherwise>
	...
	<!-- 与此页相关的内联脚本 -->
	<script src="${path}/assets/custom-js/integra/slintegration.js"></script>

                                ...
                                <table id="simple-table" class="table  table-bordered table-hover user-list">
                                    ...
                                    <tr>
                                       ...
                                       <th><i class="fa  bigger-110 hidden-480"></i>ID</th>
                                        <th><i class="fa  bigger-110 hidden-480"></i>分类描述</th>
                                        <th><i class="fa  bigger-110 hidden-480"></i>父级分类</th>
                                        <th><i class="fa  bigger-110 hidden-480"></i>方案分类简介</th>
                                    ...
                                    <shiro:hasPermission name="schemeclass:list">
                                    <c:forEach items="${list.list}" var="list">
                                            ...
                                            <td class="scinfo">${list.scinfo}</td>
														<td>
															...
															<button type="button" class="btn btn-primary btn-xs show-add-form" onclick="dele(${list.scid})" >删除分类</button>
															</div>
														</td>
                                    ...
                                    <ul class="pagination">
                                        <c:choose>
                                            <c:when test="${list.hasPreviousPage}">
                                                <li><a href="${path}/schemeclass/list?pageNum=1">首页</a></li>
                                                <li>
                                                    <a href="${path}/schemeclass/list?pageNum=${list.pageNum-1}"
                                                       aria-label="Previous">
                                        ...
                                        <c:forEach items="${list.navigatepageNums }" var="page">
                                            <c:if test="${page==list.pageNum}">
                                                <li class="active"><a
                                                        href="${path}/schemeclass/list?pageNum=${page}">${page}</a></li>
                                            </c:if>
                                            <c:if test="${page!=list.pageNum}">
                                                <li><a href="${path }/schemeclass/list?pageNum=${page}">${page}</a></li>
                                            </c:if>
                                        </c:forEach>
                                        ...
                                        <c:choose>
                                            <c:when test="${list.hasNextPage }">
                                                ...
                                                <li><a href="${path }/schemeclass/list?pageNum=${list.pages}">末页</a></li>
                                            </c:when>
                                            <c:otherwise>
                                        ...
                                        <div class="modal-body">
                                            <div class="row">
                                                <div class="col-xs-12 col-sm-6">
                                                    ...
                                                    <label for="form-add-realName" style="display: none">方案分类ID
								...
								<div class="modal-dialog" role="document">
									<div class="modal-content">
										<div class="modal-header">
											...
											<h4 class="modal-title" id="myModalLabel">新增子分类</h4>
										</div>

										<div class="modal-body">
											<form id="for" class="for" name="for">
												<div class="form-group" style="display: none">
													<label for="recipient-name" class="control-label">方案父级分类id:</label>
													<input type="text" class="form-control" id="scid11"
														name="scid" readonly unselectable="on">
												</div>
												<div class="form-group">
													<label for="message-text" class="control-label">方案父级分类:</label>
													<input class="form-control" id="scscname1" name="scscname1" readonly unselectable="on"></textarea>
												</div>
												<div class="form-group">
												...
												<div class="form-group">
													<label for="message-text" class="control-label">方案分类简介:</label>
...
<!-- 与此页相关的内联脚本 -->
<script src="${path}/assets/custom-js/schemeclass.js"></script>
...
$('#exampleModal').on('show.bs.modal', function (event) {
    ...
    $.ajax({
			url : "${path}/schemeclass/selectname",
			type : "POST",
			dataType : "json",
			data : {
			'scid' : recipient
